...
export function StudentTopicsPage() {
  ...
  const demoTopics = (topics ?? []).filter((t) => t.isDemo);
  ...
  const renderTopicCard = (topic: Topic) => {
    ...
    return (
      <Card
        ...
        onClick={() => !starting && startTopic(topic)}
      >
        <CardContent className="p-5 space-y-4">
          ...
          {answered > 0 && !locked ? (
            ...
            <div className={cn("flex items-center gap-1.5 text-xs font-medium", locked ? "text-amber-500" : "text-muted-foreground")}>
              {locked ? <><Lock className="h-3 w-3" /><span>Obuna kerak</span></> : <span>Hali o'rganilmagan</span>}
            </div>
          )}

          {isActive && <p className="text-xs text-primary font-medium">Yuklanmoqda...</p>}
        </CardContent>
      </Card>
    );
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {demoTopics.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Demo mavzular</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {demoTopics.map(renderTopicCard)}
          </div>
        </section>
      )}
      ...
      {activeTopics.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
            Barcha mavzular ({activeTopics.length})
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 14, color: "#64748b", marginBottom: 28, lineHeight: 1.6, fontFamily: "inherit" }}>
              Bu mavzudan foydalanish uchun faol obuna kerak. Demo mavzu bepul!
            </p>
